...
class SessionAnalytics:
    ...
    def calculate_roi(self) -> dict[str, Any]:
        ...
        return {
            ...
            "precision_rate": len(injections) / (len(injections) + len(misses)) if (len(injections) + len(misses)) > 0 else 1.0
        }
...
def get_global_roi(data_dir: Path) -> dict[str, Any]:
    ...
    for path in session_dir.glob("*.json"):
        try:
            ...
            data = json.loads(path.read_text(encoding="utf-8"))
            analytics = SessionAnalytics(session_id=data["session_id"], start_time=data["start_time"])
            analytics.events = [
                SessionEvent(timestamp=e["timestamp"], event_type=e["type"], details=e["details"])
                for e in data["events"]
            ]
    ...
    return {
        ...
        "historical_precision": total_injections / (total_injections + total_misses) if (total_injections + total_misses) > 0 else 1.0
    }
