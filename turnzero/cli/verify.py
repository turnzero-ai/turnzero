            ...
            with httpx.Client(timeout=1.0) as client:
                ...
                if resp.status_code == 200:
                    ...
                    if "nomic-embed-text:latest" in models or "nomic-embed-text" in models:
                        console.print("  [green]✓[/green] ollama is running with nomic-embed-text")
                    else:
                        console.print("  [yellow]⚠[/yellow] ollama is running but nomic-embed-text is missing")
                else:
                    console.print(f"  [red]✗[/red] ollama server returned error: {resp.status_code}")
        except Exception:
            console.print("  [red]✗[/red] ollama server is not reachable (check if it is running)")
...
def _verify_index_and_library(data_dir: Path) -> None:
    ...
    if blocks_dir.exists():
        ...
        console.print(f"  [green]✓[/green] Library: {n_blocks} blocks found in {blocks_dir}")
    else:
    ...
    if index_path.exists():
        try:
            with open(index_path, encoding="utf-8") as f:
                ...
                console.print(f"  [green]✓[/green] Index: Found at {index_path} (Model: {model})")
        except Exception:
            console.print(f"  [red]✗[/red] Index at {index_path} is unreadable or malformed")
    else:
...
def _verify_client_registrations(data_dir: Path) -> None:
    ...
    if platform.system() == "Darwin":
        cursor_path = Path.home() / "Library/Application Support/Cursor/User/globalStorage/saoudrizwan.claude-dev/settings/mcp_servers.json"
    ...
    _check_md_instructions(Path.home() / ".claude" / "CLAUDE.md", "Claude Code (rules)", table)
    _check_md_instructions(Path.home() / ".gemini" / "GEMINI.md", "Gemini CLI (rules)", table)
...
def _check_json_mcp(path: Path, label: str, table: Table) -> None:
    ...
    try:
        ...
        if "turnzero" in content:
            ...
            table.add_row(f"  [yellow]⚠[/yellow] {label:<20}", "[yellow]missing turnzero entry[/yellow]")
    except Exception:
...
def _check_md_instructions(path: Path, label: str, table: Table) -> None:
    ...
    try:
        ...
        if _TURNZERO_MD_MARKER in content:
            ...
            table.add_row(f"  [yellow]⚠[/yellow] {label:<20}", "[yellow]rules missing or stale[/yellow]")
    except Exception:
...
def _verify_retrieval_probe() -> None:
    ...
    probe_prompt = "Building a FastAPI REST API with Pydantic models and async SQLAlchemy"
    
    try:
        results = _list_suggested_blocks(probe_prompt)
        if results:
            names = [r["block_id"] for r in results if r.get("block_id") != "personal-priors-limit-warning"]
            console.print(f"  [green]✓[/green] Probe successful: found {len(names)} relevant blocks")
            console.print(f"    [dim]Top hits: {', '.join(names[:3])}[/dim]")
        else:
            console.print("  [red]✗[/red] Probe failed: no blocks retrieved for a known implementation prompt")
