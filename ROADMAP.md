...
## Maintenance & Done
- [x] v0.17.0 — DELIV-1 Phase 1: local HTTP proxy (`turnzero proxy serve`), compliance-free prior injection for any OAI-compatible client. Starlette ASGI, uuid4 sessions, fail-open injection, data-driven provider resolution (Anthropic, OpenAI, Google, xAI/Grok). Three-tier quality gate pipeline (pre-commit/pre-push hooks + docs-guard CI + dependabot). TST-DEBT-1 resolved. venv on py3.12. 450 tests.
