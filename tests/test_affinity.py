...
def test_session_deduplication(tmp_path: Path):
    ...
    with patch("turnzero.config._data_dir", return_value=data_dir):
        ...
        index = [
            IndexEntry(block_id=block_id, embedding=[0.1]*768, domain="fastapi", intent="build", tags=["fastapi"], source="local")
        ]
        blocks = {
            block_id: Block(slug=block_id, hash="h", version="1", domain="fastapi", intent="build", last_verified="2026", tags=["fastapi"], context_weight=100, constraints=[], anti_patterns=[], doc_anchors=[])
        }
...
def test_project_affinity_boosting(tmp_path: Path):
    ...
    with patch("turnzero.config._data_dir", return_value=data_dir):
        ...
        with patch("turnzero.retrieval.cosine_similarity", return_value=0.70):
            results = query("fastapi", index, blocks, project_root=project_root, threshold=0.75)
